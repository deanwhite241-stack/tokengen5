...
async function createToken(token) {
  const sql = `
    INSERT INTO tokens (contract_address, contract_type, name, symbol, decimals, initial_supply, max_supply, owner_address, network_id, network_name, network_chain_id, transaction_hash, verified, features, created_at) 
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW())
  `;
  await query(sql, [
...
async function updateTokenStats(address, { totalSupply, holdersCount, transferCount }) {
  const sql = `
    UPDATE tokens SET total_supply = ?, holders_count = ?, transfer_count = ?, last_updated = NOW() 
